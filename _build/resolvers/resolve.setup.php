...
if ($object->xpdo) {
	...
	switch ($options[xPDOTransport::PACKAGE_ACTION]) {
		...
		case xPDOTransport::ACTION_UPGRADE:
            ...
            $deliveries = array(
                'Почта России Простая бандероль',
                'Почта России Заказная бандероль',
                'Почта России Заказная бандероль 1 класс',
                'Почта России Ценная бандероль',
                'Почта России Ценная посылка',
                'Почта России Ценная авиабандероль',
                'Почта России Ценная авиапосылка',
                'Почта России Ценная бандероль 1 класс',
                'Почта России Курьерская доставка EMS',
            );
            $deliveriesSaved = array();

            foreach ($deliveries as $deliveryName) {

                if (!$delivery = $modx->getObject('msDelivery', array('name' => $deliveryName))) {
                    $delivery = $modx->newObject('msDelivery');
                    $delivery->fromArray(array('name' => $deliveryName,
                        'class' => 'msRussianPostHandler',
                        'requires' => 'receiver,phone,city',
                        'active' => 0));
                }

                $delivery->set('properties', array('sendingName' => trim(str_replace('Почта России', '', $deliveryName)), 'deliveryName' => $deliveryName));
                $delivery->save();

                $deliveriesSaved[$delivery->get('id')] = $deliveryName;

            }

            file_put_contents(MODX_CORE_PATH.'components/msrussianpost/deliveries.json', json_encode($deliveriesSaved));

            $oldHandler = $ms2->config['customPath'].'delivery/russianpost.class.php';
            if (file_exists($oldHandler)) unlink($oldHandler);

            if (!copy(MODX_CORE_PATH.'components/msrussianpost/libs/msrussianposthandler.class.php' , $ms2->config['customPath'].'delivery/msrussianposthandler.class.php')) {
                $modx->log(xPDO::LOG_LEVEL_ERROR, 'msRussianPost Handler class not copied to minishop2 folder');
            }

			if ($menu = $modx->getObject('modMenu', array('text' => 'msRussianPost'))) $menu->remove();
